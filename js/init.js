  ...
  "use strict";
	...
	var vkm = {
		...
		init: function(){
			...
			vkm.navigation__resize();

			

			/* Navigation Closer */

			vkm.navigation__closer();

			

			/* Smooth Scroll to next section */

			vkm.scrollToNextSection();
		...
		mobileMenuOpener: function(){
			...
			var hamburger		= $('.vkm_fn_mobilemenu_wrap .hamburger');

			hamburger.on('click',function(){

				var element 	= $(this);

				var menupart	= $('.vkm_fn_mobilemenu_wrap .mobilemenu');
		...
		colorScheme: function(){
			...
			var scheme 		= $('.vkm_fn_color_scheme');	// color scheme
			...
			var activeColor = vkm.root.css(mainColor).replace(/\s+/g, '');	// detect main color to change it in next functions
			...
			button.on('click',function(){
				...
				if(!element.hasClass('active')){
					...
					if(color !== '' && color !== '#'){
						...
						vkm.root.css(mainColor,color);

						activeColor = color;

						vkm.setTextColor(color,textColor);
			...
			opener.off().on('click',function(e){
				...
				vkm.root.css(mainColor, activeColor);

				vkm.setTextColor(activeColor,textColor);
		...
		setTextColor: function(color,textColor){
			...
			if (vkm.returnLuma(color) < 60) {

				vkm.root.css(textColor,'#fff');

			}else{

				vkm.root.css(textColor,'#000');
		...
		magnific: function(){
			...
			$('.gallery_zoom').each(function() { // the containers for all your galleries
				...
				var galleryItems 	= vkm.magnificItems(zoom);
		...
		movingPlaceholder: function(){
			...
			$('.vkm_fn_contact .input_wrapper').each(function(){
		...
		contactForm: function(){
			...
			$('#send_message').on('click', function(){
				...
				var form		= $('.vkm_fn_contact .contact_form');
		...
		loadBlogPosts: function(){
			...
			$('.blog_list .load_more a').on('click',function(){
				...
				for(var i=0;i<count;i++){
					...
					vkm.search_placeholder(text,newText,i,speed);
		...
		progress: function(){
			...
			$('.fn_cs_progress_bar').each(function() {
				...
				pWrap.waypoint({handler: function(){vkm.progressF(pWrap);},offset:'90%'});
		...
		navigation__closer: function(){
			...
			var wrapper		= $('.vkm_fn_wrapper_all');

			$('.vkm_fn_sidebar .nav__button').off().on('click',function(){
		...
		navigation__resize: function(){
			...
			var sidebar		= $('.vkm_fn_sidebar'),

				content 	= $('.vkm_fn_content'),
